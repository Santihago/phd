---
title: "R Analysis Notebook: Subjective and cardiac responses, Perruchet"
...
psypy_path <- '/Users/santiago/Dropbox/MyScience/MyPhD/MyPhD-projects/PC/exp_1/data/behavData/'
...
# folder containing individual datasets 
ecg_path <- "/Users/santiago/Dropbox/MyScience/MyPhD/MyPhD-projects/PC/exp_1/analysis/preprocessed/CS/interpolated/"
...
ecg_rest_path <- "/Users/santiago/Dropbox/MyScience/MyPhD/MyPhD-projects/PC/exp_1/analysis/preprocessed/rest"
...
data_long_clean %>%
  ...
  ggplot(aes(y=bpm_clean, x = tp, group=condition, color=condition)) +
    ...
    labs(
       ...
       y = "BPM") +
    annotate("rect", xmin = -3.15, xmax=-3, ymin = 67.5, ymax=68.5, alpha=.2, fill = "red") + 
    annotate("rect", xmin = 0., xmax=.5, ymin = 67.5, ymax=68.5, alpha=.2) + 
    annotate("rect", xmin = 4., xmax=4.5, ymin = 67.5, ymax=68.5, alpha=.2) + 
    annotate("text", x = -4, y = 68, label = "Rating", color = "black") + 
    annotate("text", x = .85, y = 68, label = "E1", color = "black") + 
    annotate("text", x = 5, y = 68, label = "E2", color = "black") + 
    NULL
...
#cardiac_trial_data_noNA %>%
#  lmer(hr ~ cond_lag * count_lag + (1|id), 
#       data=., 
#       REML=F, 
#        control=lmerControl(optimizer="bobyqa",
#                                 optCtrl=list(maxfun=2e5))) %>%
...
```{r eval=F}
data1a <- cardiac_trial_data_noNA %>%
  mutate(id=id) 

write.csv(data1a, file = "data_hr_e1_exp1.csv" ,row.names = T)
```
...
cardiac_long_simple <- 
  cardiac_long_simple %>%
    drop_na() %>%
    mutate(model_pred = predict(mm_fit7))
...
```{r eval=F}
data1 <- cardiac_trial_us_data_noNA %>%
  mutate(id=id) 

write.csv(data1, file = "data_hr_e2_exp1.csv" ,row.names = T)
```
